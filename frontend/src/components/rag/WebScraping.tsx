...
const WebScraping: React.FC = () => {
  ...
  const loadSites = async () => {
    try {
      const response = await fetch('/api/web-scraping-v2/documents');
      const data = await response.json();
      console.log("DEBUG: Sites loaded from API:", data);
      setSites(data || []);
    } catch (error) {
  ...
  const loadStats = async () => {
    try {
      const response = await fetch('/api/web-scraping-v2/stats');
      const data = await response.json();
      setStats(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/web-scraping-v2/scrape', {
        method: 'POST',
        headers: {
  ...
  const deleteSite = async (siteUrl: string) => {
    ...
    try {
      const response = await fetch('/api/web-scraping-v2/document', {
        method: 'DELETE',
            ...
            {sites.map((site) => (
              <div key={site.url} className="flex items-center justify-between p-3 bg-gray-50 rounded border">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">Scrappato: {new Date(site.scraped_at).toLocaleString()}</p>
                </div>
                <button
